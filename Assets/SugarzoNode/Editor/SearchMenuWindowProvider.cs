...
namespace SugarFrame.Node
{
    public class SearchMenuWindowProvider : ScriptableObject, ISearchWindowProvider
    {
        public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
            ...
            {
                string str = NodePackageType.Base.ToString();
                if (type.IsDefined(typeof(NodeNoteAttribute), true))
                    str = (System.Attribute.GetCustomAttribute(type, typeof(NodeNoteAttribute)) as NodeNoteAttribute).packageType.ToString();
                return str;
            };

            //遍历所有包类型
            Enum.GetNames(typeof(NodePackageType)).ToList<string>().ForEach(packageStr => {
